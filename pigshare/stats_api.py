...
def add_totals_method(cls, stats_type, item_type):
    ...
    def totals_method(self, id):
        ...
        totals_json = json.loads(response.body_string())
...
def add_timeline_method(cls, stats_type, item_type):
    ...
    def timeline_method(self, id, granularity="total", start_date=None, end_date=None, sub_item=None, sub_item_id=None):
        ...
        timeline_json = json.loads(response.body_string())
...
def add_breakdown_method(cls, stats_type, item_type):
    ...
    def breakdown_method(self, id, granularity="total", start_date=None, end_date=None, sub_item=None, sub_item_id=None):
        ...
        if self.institution:
            response = self.get('/{0}/breakdown/{1}/{2}/{3}/{4}'.format(self.institution, granularity, stats_type, item_type, id), headers=get_headers(token=self.token), params_dict=get_request_params(start_date=start_date, end_date=end_date, sub_item=sub_item, sub_item_id=sub_item_id))
        else:
            response = self.get('/breakdown/{0}/{1}/{2}/{3}'.format(granularity, stats_type, item_type, id), headers=get_headers(token=self.token), params_dict=get_request_params(start_date=start_date, end_date=end_date, sub_item=sub_item, sub_item_id=sub_item_id))

        breakdown_json = json.loads(response.body_string())
...
class figshare_stats_api(Resource):
    ...
    def __init__(self, stats_url=STATS_DEFAULT_URL, institution=None, token=None, verbose=False, **kwargs):
